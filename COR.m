...
combinations = [
    ...
    0.3000    10.0000    0.1000
    0.3000    40.0000    0.1000
    0.3000    40.0000    0.0150
    
    0.1000    10.0000    0.1000
    0.1000    40.0000    0.1000
];
...
if ~exist('dat', 'var') % Data file does not exist, most likely first session
    ...
    variableNames = {...
        ...
        'trialStatus', 'rawOriError', 'trlError'
        };
    ...
    dat = table( ...
            ...
            trlStatusVec, rawOriErrorVec, trlErrorVec, ...
...
try
    ...
    for blockIDx=startBlockIdx:numBlocks
        ...
        colsToPrint = {'stimOri', 'stimDur', 'stimSpread', 'stimContrast', 'reportedOri', 'reportedConf', 'rawOriError', 'trlError'};
%         headers = dat(1, colsToPrint).Properties.VariableNames;
%         fprintf('%5s  %5s  %5s  %5s  %5s  %5s  %5s  %5s', headers{:});
%         fprintf("\n")
        ...
        % while nCompletedTrialsCurrBlock < 10
            ...
            DrawFormattedText(psychToolBoxConfig.w, ['' num2str(currentTotalRewardPoints, '%.2f')], ...
            ...
            %fprintf('%5s  %5s  %5s  %5s  %5s  %5s  %5s  %5s', num2str(dat{trlCfgIdx, colsToPrint}));
            %fprintf("\n")
            disp(dat{trlCfgIdx, colsToPrint})
        end
        ...
        % Save info and print block stats
        thisBlockData = dat( (dat.session == sessionNum) & (dat.block == blockIDx), :);
        stimOri = thisBlockData.stimOri; reportedOri = thisBlockData.reportedOri;
        ...
        thisBlockData.absOriError = absOriError;
        thisBlockData.rawOriError = rawOriError;
        
        summaryTable1 = groupsummary(thisBlockData, {'reportedConf'}, ...
                            {@(x) sqrt(nanmean(x.^2)), 'mean', 'std', 'numel'}, 'rawOriError');

        summaryTable2 = groupsummary(thisBlockData, {'reportedConf'}, ...
        ...
        fprintf('Percent Correct (positive reward): %.2f', sum(thisBlockData.trlError)/numel(thisBlockData.trlError))
        fprintf("\n")    
        disp(summaryTable1)
        disp(summaryTable2)
        ...
        endOfBlockScreen(psychToolBoxConfig, currentTotalReward) % Show waiting screen
...
function endOfBlockScreen(psychToolBoxConfig, rewardUSD)
