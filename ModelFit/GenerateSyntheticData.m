...
sigma_e_levels   = [8.37, 10.53, 11.93, 13.52, 15.22, 16.32]; %linspace(0.01, 2, 10);  % Varying external noise or contrast
% sigma_e_levels   = [11.4211   12.6518   14.1961   14.5538   14.3205   15.6123];
sigma_i          = 12;
% sigma_i          = 10.2227;
varGain          = 0.5;
% varGain          = 0.7688;
...
for i = 1:numel(sigma_e_levels)
    subplot(2, numel(sigma_e_levels)/2, i)
    err = resp_err_all_reshaped(i, :)';
    histogram(err, -90:3:90, Normalization="pdf")
    ...
    shape = 1/varGain;
    scale = varGain;
    a_stddev = getAnalyticalSigma(sigma_e_levels(i), sigma_i, scale, shape);
    
    xlabel("Orientation (deg)")
    ylabel("count")
    title(sprintf("External noise level %.2f \n fit std %.2f, std %.2f\n analytical std: %.2f", ...
        sigma_e_levels(i), sigma, val, a_stddev))
...
sigma_e          = 21; %linspace(0.01, 2, 10);  % Varying external noise or contrast
sigma_i          = 12;
varGain          = 0.5;
sigma_m          = 0.2;
Cc               = 0.5;
...
shape = 1/varGain;
scale = varGain;
analyticalStd = getAnalyticalSigma(sigma_e, sigma_i, scale, shape);

analyticalStds(h, itr) = analyticalStd;
...
%%
function stddev = getAnalyticalSigma(sigma_e, sigma_i, scale, shape)
stddev = sqrt(sigma_e^2 + sigma_i^2*scale^2*shape*(shape + 1));
end
