%%%
% Contrast of 0.015 seems to be opearting very near to the threshold. 
% Probably this is why HC reports are super precise and LC reports are super dispersed.

% Time and contrast also probably interacting in a non linear way
%%%
close all
clear all

% data = load('COR14.mat'); % dur, contrast, dispersion
data = load('COR15.mat'); % Changed reward function - affects perceptual variance
% data = load('COR16.mat'); % orientation dependent reward
% data = load('COR17.mat'); % ori dependent reward - changed reward function c1 = 5
% COR18 - c1=1, c2=0.3, -3 (all HC)
% COR19 - c1=1, c2=0.3, -0.5 (all HC)

% stimContrast = data.dat.stimContrast;
% sctstFltIDx = stimContrast ~= 0.015;
% data.dat = data.dat(sctstFltIDx, :);

% stimContrast = data.dat.stimDur;
% sctstFltIDx = stimContrast == 0.3;
% data.dat = data.dat(sctstFltIDx, :);
...
xlabel("Uncertainty level (Dur, C, Spread)", 'FontSize', 14)
