...
xLabels = strcat(string(sContrast), ', ', string(sSpread));
...
for i=1:numel(sidx)
    ...
    xlabel("Orientation (deg)")
    ylabel("count")
    title(sprintf("Contrast: %.2f, \n Spread: %.2f", contrastLevels(gidx_), spreadLevels(gidx_)))
    % xlim([-50, 50])
end
...
sSpread = summaryTable.stimSpread;
[B_, sidx] = sort(sSpread - sContrasts);

sContrast = sContrasts(sidx);
sSpread = sSpread(sidx);
